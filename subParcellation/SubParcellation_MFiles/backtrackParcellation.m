...
path_new_annot=[subject 'label/temp/' hemi '.' int2str(surface) '_temp.aparc.annot'];
...
[vertices2, label2, colortable2]=read_annotation(['/home/rr480/FSFiles/fsaverageSubP/label/' hemi '.aparc.annot']);
valid=[]
for ie=1:numel(colortable.struct_names)
     name=colortable.struct_names(ie);
     valid(ie)=not(isempty(name{1}));
end
colortable.numEntries=sum(valid);
colortable.struct_names=colortable.struct_names(find(valid));
colortable.table=colortable.table(find(valid),:);
